...
export const analyzeResume = async (req, res) => {
  try {
    ...
    const message = [
      {
        role: "system",
        content: `Extract structured date from resume.
    ...
    const cleanOutput = aiResponse.replace(/```(?:json)?/gi, "").trim();
...
export const generateQuestions = async (req, res) => {
  try {
    ...
    projects = projects?.map((p) => p?.trim());
    ...
    if (user.credit < 50) {
      return res.status(404).json({ message: "Not enough credits" });
    }
    ...
    const messages = [
      {
        ...
        projects, and skills, and resume details.
        `,
      },
      {
        role: "user",
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    const messages = [
      {
        ...
        - Consider clarity, structure, nad relevance.
        
        Calculate:
        Final Score = average of confidence, communication, and correctness (rounded to
        ...
        - Can suggest imporvement if needed.
    ...
    try {
      ...
      console.error("Failed to parse AI response as JSON in submitAnswer:", cleanOutput);
      throw new Error("AI returned invalid data format for answer evaluation.");
    }
